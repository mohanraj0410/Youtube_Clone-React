...
const SearchVideo = () => {
    ...
    const { searchVideoId } = useParams()

    const fetchData = async () => {
        if (topDiv.current) {
            topDiv.current.scrollIntoView({ behavior: 'smooth' })
        }
        const url = `https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&id=${searchVideoId}&key=${API_KEY}`
        await fetch(url)
            .then(res => res.json())
    ...
    const fetchCommentData = async () => {
        const url = `https://youtube.googleapis.com/youtube/v3/commentThreads?part=snippet%2Creplies&videoId=${searchVideoId}&key=${API_KEY}`
        await fetch(url)
            .then(res => res.json())
    ...
    return (
        <>
            ...
            <div className='play-search-video'>
                <iframe src={`https://www.youtube.com/embed/${searchVideoId}?autoplay=1`} frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
